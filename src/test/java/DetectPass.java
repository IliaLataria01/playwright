...
public class DetectPass {
    ...
    @Test
    public void detectPassessOfSpecificPage() {
    ...
    private static void outputAccessibilityResults(AxeResults accessibilityScanResults) {
        if (accessibilityScanResults.getViolations().isEmpty()) {
            System.out.println("No accessibility passes found.");
        } else {
            System.out.println("Accessibility passes found:");
            FileUtils.writeToFile("src/test/resources/single_violations/accessibility_passes.json",accessibilityScanResults.getPasses());
        }
    }
}
